...
export default function PropertyAuthoritiesPage() {
  ...
  ): UseMutationOptions<ApiResponse<PropertyAuthorityClaim>, Error, PropertyAuthorityClaim> => ({
  ...
  // The options are built per action; the hooks themselves are called here,
  // unconditionally, because that is what the rules of hooks require.
  const approve = useMutation(decideOptions('approve'));
  const reject = useMutation(decideOptions('reject'));
  const revoke = useMutation(decideOptions('revoke'));
