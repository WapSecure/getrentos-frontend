...
export const isRenterRouteActive = (pathname: string, href: string) =>
  pathname === href || (href !== ROUTES.RENTER_DASHBOARD && pathname.startsWith(`${href}/`));
...
export const RenterSidebar = () => {
  ...
  return (
    <aside className="fixed left-0 top-16 bottom-0 z-30 hidden w-64 overflow-y-auto border-r border-border/70 bg-card/55 backdrop-blur-xl supports-backdrop-filter:bg-card/65 lg:block">
      <nav className="space-y-5 p-4" aria-label="Renter navigation">
        {navGroups.map((group) => (
          <div key={group.label}>
            <h2 className="mb-1.5 px-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/75">
              {group.label}
            </h2>
