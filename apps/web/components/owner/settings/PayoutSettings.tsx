...
export const PayoutSettings = () => {
  ...
  useEffect(() => {
    const fetchPayoutAccount = async () => {
      const response = await ownerService.getPayoutSettings();
  ...
  const handleSave = async () => {
    if (!canSave) return;
    setSubmitError(null);
    const response = await ownerService.updatePayoutSettings({ bankCode, accountNumber });
    if (response.success && response.data) {
      setSavedBankName(response.data.bankName);
      setSavedAccountNumber(response.data.accountNumber);
      setSavedAccountName(response.data.accountName);
      ...
      setSubmitError(response.message || 'Unable to update your payout account.');
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Where escrow-released sale proceeds are sent — transfers run through Paystack.
      </p>
