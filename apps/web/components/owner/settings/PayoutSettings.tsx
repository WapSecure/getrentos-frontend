...
export const PayoutSettings = () => {
  ...
  const { data: account } = useQuery({
    queryKey: ownerKeys.payoutSettings,
    queryFn: () => unwrap(ownerService.getPayoutSettings()),
  });
  ...
  const save = useMutation({
    mutationFn: () =>
      unwrap(ownerService.updatePayoutSettings({ bankName, accountNumber, accountName })),
    onSuccess: (result) => {
      setVerified(result.verified);
      queryClient.invalidateQueries({ queryKey: ownerKeys.payoutSettings });
