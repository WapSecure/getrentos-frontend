'use client';

import { useState } from 'react';
import { Star, MessageCircle, Clock, ClipboardCheck } from 'lucide-react';
import { getInitials, formatDate } from '@/lib/format';

interface ClientReview {
  id: string;
  reviewerName: string;
  reviewerRole: 'Client' | 'Admin';
  taskLabel: string;
  rating: number;
  communication: number;
  thoroughness: number;
  timeliness: number;
  comment: string;
  createdAt: string;
}

const mockReviews: ClientReview[] = [
  {
    id: 'rev_001',
    reviewerName: 'Segun Alabi',
    reviewerRole: 'Client',
    taskLabel: 'Property Valuation Visit',
    rating: 5,
    communication: 5,
    thoroughness: 5,
    timeliness: 4,
    comment: 'Very thorough inspection with clear photos and notes. Arrived right on time.',
    createdAt: '2026-07-20T00:00:00.000Z',
  },
  {
    id: 'rev_002',
    reviewerName: 'GetRentos Admin',
    reviewerRole: 'Admin',
    taskLabel: 'Move-out Inspection — Ikeja GRA',
    rating: 5,
    communication: 4,
    thoroughness: 5,
    timeliness: 5,
    comment: 'Detailed report submitted same day, synced without issues.',
    createdAt: '2026-06-10T00:00:00.000Z',
  },
];

const StarRow = ({ rating }: { rating: number }) => (
  <div className="flex items-center gap-0.5">
    {Array.from({ length: 5 }).map((_, i) => (
      <Star
        key={i}
        className={`w-3.5 h-3.5 ${i < rating ? 'fill-primary text-primary' : 'text-muted-foreground/30'}`}
      />
    ))}
  </div>
);

export default function AgentReviewsPage() {
  const [reviews, setReviews] = useState<ClientReview[]>(mockReviews);

  const avg = (
    key: keyof Pick<ClientReview, 'rating' | 'communication' | 'thoroughness' | 'timeliness'>
  ) => (reviews.length ? reviews.reduce((sum, r) => sum + r[key], 0) / reviews.length : 0);

  const overallRating = avg('rating');
  const categories = [
    { label: 'Communication', icon: MessageCircle, value: avg('communication') },
    { label: 'Thoroughness', icon: ClipboardCheck, value: avg('thoroughness') },
    { label: 'Timeliness', icon: Clock, value: avg('timeliness') },
  ];

  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Reviews & Reputation</h1>
        <p className="text-muted-foreground mt-1">See how clients and admins rate your fieldwork</p>
      </div>

      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        <div className="bg-card rounded-2xl border border-border p-6 flex flex-col items-center justify-center text-center">
          <p className="text-4xl font-bold text-foreground">{overallRating.toFixed(1)}</p>
          <StarRow rating={Math.round(overallRating)} />
          <p className="text-xs text-muted-foreground mt-2">
            Based on {reviews.length} review{reviews.length === 1 ? '' : 's'}
          </p>
        </div>

        <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-6 space-y-4">
          {categories.map((category) => (
            <div key={category.label} className="flex items-center gap-3">
              <category.icon className="w-4 h-4 text-gray-400 shrink-0" />
              <span className="text-sm text-muted-foreground w-36 shrink-0">{category.label}</span>
              <div className="flex-1 h-2 rounded-full bg-secondary overflow-hidden">
                <div
                  className="h-full rounded-full bg-primary"
                  style={{ width: `${(category.value / 5) * 100}%` }}
                />
              </div>
              <span className="text-sm font-medium text-foreground w-8 text-right">
                {category.value.toFixed(1)}
              </span>
            </div>
          ))}
        </div>
      </div>

      <div className="space-y-4">
        {reviews.map((review) => (
          <div key={review.id} className="bg-card rounded-2xl border border-border p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-linear-to-r from-primary to-primary/60 flex items-center justify-center text-primary-foreground font-semibold text-xs shrink-0">
                  {getInitials(review.reviewerName)}
                </div>
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {review.reviewerName}{' '}
                    <span className="text-xs text-gray-400 font-normal">
                      · {review.reviewerRole}
                    </span>
                  </p>
                  <p className="text-xs text-muted-foreground">{review.taskLabel}</p>
                </div>
              </div>
              <div className="text-right shrink-0">
                <StarRow rating={review.rating} />
                <p className="text-xs text-gray-400 mt-1">{formatDate(review.createdAt)}</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-3">{review.comment}</p>
          </div>
        ))}
      </div>
    </>
  );
}
