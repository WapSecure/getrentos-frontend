...
export default function BuyerLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      if (storedUser) {
        const parsedUser: BuyerUser = JSON.parse(storedUser);

        // A user can hold multiple roles (e.g. Renter + Buyer); gate on whether
        // ANY of them is buyer-equivalent, not just the primary `role` field.
